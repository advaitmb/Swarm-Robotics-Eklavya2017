 ...
 /* Client client = server.available();
  ...
  for (int x = 0; x < video.width; x++ ) {
    for (int y = 0; y < video.height; y++ ) {
      ...
      if(h2>2){
      if(abs(h2-h1)<2 && abs(s2-s1)<15&&abs(b2-b1)<15){
        trueRed = true;
      }else trueRed = false;
      }else trueRed = false;
      
      if(h3>2){
      if(abs(h3-h1)<2 && abs(s3-s1)<15 && abs(b3-b1)<15){
        trueGreen = true;
      }else trueGreen = false;
      }else trueGreen = false;
      
      if(h4>2){
      if(abs(h4-h1)<2 && abs(s4-s1)<10&&abs(b4-b1)<20){
        trueBlue = true;
      }else trueBlue = false;
      }else trueBlue = false;
...
boolean checkCollision(){
  if(distSq(red.centroid_x,red.centroid_y,green.centroid_x,green.centroid_y)<100*100 ){
    collision = true;
  }
  else {
    collision = false;
...
void avoidCollision(){
   ...
   if(distSq(red.centroid_x,red.centroid_y,green.centroid_x,green.centroid_y)<100*100) /*distSq(red.centroid_x,red.centroid_y,blue.centroid_x,blue.centroid_y)>40*40
